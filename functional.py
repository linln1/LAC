...
def drop_feature(x: torch.Tensor, drop_prob: float) -> torch.Tensor:
    device = x.device
    drop_mask = torch.empty((x.size(1),), dtype=torch.float32, device=device).uniform_(0, 1) < drop_prob
    x = x.clone()
    x[:, drop_mask] = 0

    return x

def COSTA_feature(x: torch.Tensor, drop_prob: float) -> torch.Tensor:
    device = x.device
    drop_mask = torch.empty((x.size(0),), dtype=torch.float32, device=device).uniform_(0, 1) < drop_prob
    k = torch.tensor(int(x.shape[0] * drop_prob))
    p = (1/torch.sqrt(k))*torch.randn(x.shape[0], x.shape[0]).to(device)
    x = x.clone()
    x[drop_mask, :] = 0
    x = p @ x 
    return x
...
def drop_feature_whole_channel_by_weight(x, weights, drop_prob: float, threshold: float = 0.7):
    '''
    :param x: [N, C]
    :param weights: [C]
    :param drop_prob:
...
def get_eigenvector_weights(data):
    ...
    return normalize(s_col), normalize(evc)

def get_eigenvector_weights_for_feat(data):
    def _eigenvector_centrality(data):
        graph = to_networkx(data)
        x = nx.eigenvector_centrality_numpy(graph)
        x = [x[i] for i in range(data.num_nodes)]
...
def get_degree_weights(data): #  for edge 
    num_nodes = data.x.size(0)
    edge_index_ = to_undirected(data.edge_index) # [2, 13264] 
    deg = degree(edge_index_[1]) # 
    deg_col = deg[data.edge_index[1]].to(torch.float32) 
    scaled_deg_col = torch.log(deg_col)

    return normalize(scaled_deg_col), normalize(deg)

def get_degree_weights_for_feat(data): # for feat
    num_nodes = data.x.size(0)
    edge_index_ = to_undirected(data.edge_index) # [2, 13264]
    deg = degree(edge_index_[1]) # 
...
def get_pagerank_weights(data, aggr: str = 'sink', k: int = 10):
    ...
    return normalize(s), normalize(pv)

def get_pagerank_weights_for_feat(data, aggr: str = 'sink', k: int = 10):
    def _compute_pagerank(edge_index, damp: float = 0.85, k: int = 10):
        num_nodes = edge_index.max().item() + 1
        deg_out = degree(edge_index[0])
        x = torch.ones((num_nodes,)).to(edge_index.device).to(torch.float32)
...
def drop_edge_by_weight(edge_index, weights, drop_prob: float, threshold: float = 1.0):
    weights = weights / weights.mean() * drop_prob
    weights = weights.where(weights < threshold, torch.ones_like(weights) * threshold) # 
    remain_mask = torch.bernoulli(1. - weights).to(torch.bool)

    return edge_index[:, remain_mask].squeeze(1)
...
class AdaptivelyAugmentTopologyAttributes(object):
    ...
    def __call__(self, x, edge_index):
        ...
        x = drop_feature_whole_channel_by_weight(x, self.feature_weights, self.pf, self.threshold)
...
def compute_heat(edge_index, edge_weight=None, temperature=5, eps=0.1, ignore_edge_attr=True, add_self_loop=True):
    ...
    diff_mat = GDC().diffusion_matrix_exact(
        edge_index, edge_weight, N, method='heat', t=temperature)
...
def perturb_edge(edge_index: torch.Tensor, ratio: float) -> torch.Tensor:
    """
    """
    ...
    remained_edge_index = dropout_adj(edge_index, p=ratio)[0]
    # mask = torch.randperm(num_edges)[num_remove:] # 
    # remained_edge_index = edge_index[:, mask].to(device) # 

    add_edge_index = torch.randint(0, num_nodes - 1, size=(2, num_add), dtype=torch.long).to(edge_index.device) # 
    merged_edge_index = torch.cat([remained_edge_index, add_edge_index], dim=1)

    edge_index = sort_edge_index(merged_edge_index)
