...
def _similarity(h1: torch.Tensor, h2: torch.Tensor):
    h1 = F.normalize(h1)
    h2 = F.normalize(h2)
    return h1 @ h2.t() # torch.mm(h1, h2.t())
...
class InfoNCESP(Loss):
    ...
    def compute(self, anchor, sample, pos_mask, neg_mask, *args, **kwargs):
        ...
        neg_mask = 1 - pos_mask   # pos.mask = torch.eye(anchor.size(0), dtype=torch.float32, device=device)
...
class InfoNCE(Loss):
    ...
    def compute(self, anchor, sample, pos_mask, neg_mask, *args, **kwargs):
        # in original grace version ,the h1, h2 use F.normalize(h1), F.normalize(h2)
