...
class AsymmDualBranchContrast(torch.nn.Module):
    def __init__(self, loss: Loss, mode: str, intraview_negs: bool = False, **kwargs):
        super(AsymmDualBranchContrast, self).__init__()
        self.loss = loss
        self.mode = mode
        self.sampler = get_sampler(mode, intraview_negs=intraview_negs)
        self.kwargs = kwargs
...
class NeiDualBranchContrast(torch.nn.Module):
    def __init__(self, loss: Loss, mode:str, intraview_neighbor_negs: bool=False, **kwargs) -> None:
        super(NeiDualBranchContrast, self).__init__()
        self.nei_con_loss = loss
        self.mode = mode
        self.sampler = get_sampler(mode, intraview_neighbor_negs=intraview_neighbor_negs)   # 
        self.kwargs = kwargs  # 

    # def nei_con_loss(self, z1: torch.Tensor, z2:torch.Tensor, pos_mask, neg_mask, adj):
    #     adj = adj - torch.diag_embed(adj.diag())  # 
    #     adj[adj>0] = 1
    #     num_neighbors = torch.sum(adj, 1) * 2 + 1   # intra-view + inter-view + self inter-view 
    #     num_neighbors = torch.squeeze(torch.tensor(num_neighbors))  # (N, 1)
    ...
    #     loss = loss / num_neighbors  # divided by the number of positive pairs for each node

    #     return -torch.log(loss)

    def forward(self, h1=None, h2=None, g1=None, g2=None, batch=None, h3=None, h4=None,
                extra_pos_mask=None, extra_neg_mask=None, adj=None):
        if self.mode == 'L2L': # 
            assert h1 is not None and h2 is not None
            anchor1, anchor2, pos_mask1, neg_mask1 = self.sampler(anchor=h1, sample=h2)
            anchor2, sample2, pos_mask2, neg_mask2 = self.sampler(anchor=h2, sample=h1)
        elif self.mode == 'G2G':
            assert g1 is not None and g2 is not None
            anchor1, sample1, pos_mask1, neg_mask1 = self.sampler(anchor=g1, sample=g2)
            anchor2, sample2, pos_mask2, neg_mask2 = self.sampler(anchor=g1, sample=g2)
        else: # G2L
            if batch is None or batch.max().item() + 1 <= 1:  # single graph
                assert all(v is not None for v in [h1, h2, g1, g2, h3, h4])
                anchor1, sample1, pos_mask1, neg_mask1 = self.sampler(anchor=g1, sample=h2, neg_sample=h4)
        ...
        pos_mask1, neg_mask1 = add_extra_mask(pos_mask1, neg_mask1, extra_pos_mask, extra_neg_mask)
        pos_mask2, neg_mask2 = add_extra_mask(pos_mask2, neg_mask2, extra_pos_mask, extra_neg_mask)
        l1 = self.nei_con_loss(anchor=anchor1, sample=sample1, pos_mask=pos_mask1, neg_mask=neg_mask1, **self.kwargs)
        l2 = self.nei_con_loss(anchor=anchor2, sample=sample2, pos_mask=pos_mask2, neg_mask=neg_mask2, **self.kwargs)
